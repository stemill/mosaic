...
case class ST_Difference(leftGeom: Expression, rightGeom: Expression, geometryAPIName: String)
    ...
    override def nullSafeEval(input1: Any, input2: Any): Any = {
        ...
        val result = geom1.difference(geom2)
        ...
        nullSafeCodeGen(
          ...
          (leftEval, rightEval) => {
              ...
              geometryAPIName match {
                  case "ESRI" =>
                      val operatorDifferenceClass = classOf[OperatorDifference].getName
                      ...
                      s"""
                         |$leftInCode
                         |$rightInCode
                         |$operatorDifferenceClass $operatorName = ($operatorDifferenceClass) $operatorFactoryLocalClass.getInstance().getOperator(
                         |    $esriOperatorClass.Type.Difference
                         |);
                  ...
                  case "JTS" =>
                      ...
                      s"""
                         ...
                         |$leftInCode
                         |$rightInCode
                         |$jtsGeometryClass $resultGeom = $leftGeomInRef.difference($rightGeomInRef);
                         |$outCode
                         |${ev.value} = $outGeomRef;
                         |} catch (Exception e) {
                         | throw e;
