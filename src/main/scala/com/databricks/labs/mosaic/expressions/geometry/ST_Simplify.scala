...
case class ST_Simplify(inputGeom: Expression, td: Expression, geometryAPIName: String) extends BinaryExpression with NullIntolerant {

    /**
      * ST_Simplify expression returns are covered by the
    ...
    override def nullSafeEval(input1: Any, input2: Any): Any = {
        ...
        val tolerance = input2.asInstanceOf[Double]
        val result = geom.simplify(tolerance)
    ...
    override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode =
        nullSafeCodeGen(
          ...
          (leftEval, rightEval) => {
              ...
              val (code, result) = GeometryTransformationsCodeGen.simplify(ctx, leftEval, rightEval, inputGeom.dataType, geometryAPI)
...
object ST_Simplify {
    ...
    def registryExpressionInfo(db: Option[String], name: String): ExpressionInfo =
        new ExpressionInfo(
            ...
            name,
            """
              |    _FUNC_(expr1) - Simplifies a given geometry.
            """.stripMargin,
            "",
            """
              |    Examples:
